...
def test_answerl10n_crud_operations(session):
    ...
    # Read
    read_answer = session.query(AnswerL10n).filter_by(id=1, aid=2).first()
    ...
    read_answer.aid = 3
    session.commit()
    updated_answer = session.query(AnswerL10n).filter_by(id=1, aid=3).first()
    assert updated_answer.answer == "Updated Answer"

    # Delete
    session.delete(updated_answer)
    session.commit()
    deleted_answer = session.query(AnswerL10n).filter_by(id=1).first()
...
def test_answer_crud_operations(session):
    ...
    # Read
    read_answer = session.query(Answer).filter_by(aid=1, qid=2).first()
    ...
    read_answer.qid = 3
    session.commit()
    updated_answer = session.query(Answer).filter_by(aid=1, qid=3).first()
    assert updated_answer.code == "ABCDE"

    # Delete
    session.delete(updated_answer)
    session.commit()
    deleted_answer = session.query(Answer).filter_by(aid=1, qid=3).first()
    assert deleted_answer is None
